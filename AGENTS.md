...
3. **Do not wrap — or redraw — a primitive that already exists.** Re-exporting `Card` with a `cn`
   ...
   **This registry now publishes its own primitives**, so "what shadcn ships" is two lists. The
   ...
   upstream primitives the web tier needs — `alert-dialog`, `command`, `item`, `separator`,
   `skeleton`, `table` (`empty` was one, and is native-first now). **Check those two directories before you write markup**; a shell
