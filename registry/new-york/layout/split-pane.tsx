...
/**
 * How much of the split the rail claims.
 *
 ...
 * shoves the rail off the screen. This is the track-level half of rule 4 — `min-w-0` on the
 * cells is the item-level half, and both are needed, since a floored track still holds an
 * unfloored item that can overflow it.
 */
const TRACKS: Record<RailWidth, [string, string]> = {
...
/**
 * The width at which the rail comes alongside instead of underneath.
 *
 ...
 */
const SPLIT_AT = {
  always: "grid-cols-[var(--cube-split-cols)]",
 ...
 * row between the same two stacked. Keyed by the same breakpoint as {@link SPLIT_AT} so the two
 * can never disagree about where the layout flips.
 */
const DIVIDER_AT: Record<keyof typeof SPLIT_AT, string> = {
  always: "h-auto w-px",
...
type SplitPaneProps = {
  ...
  content: ReactNode;
  /**
   * The second surface: a nav rail, an inspector, a note list, an order panel.
   *
   * Absent, the pane is one full-width column and neither a rail cell nor a divider is drawn.
   ...
   */
  rail?: ReactNode;
  /** Which side the rail sits on. Stacked, it keeps this reading order rather than jumping. */
  railSide?: "start" | "end";
  /** {@link RailWidth}. Naming the width is what stops eight call sites each inventing one. */
  railWidth?: RailWidth;
  /**
   * Below this width the two stack rather than sit side by side. `always` never stacks — an icon
   * rail, a kiosk, a pane already inside a media query the caller owns.
   *
   * Stacking, not hiding, is the narrow-width answer: a phone has no room for two panes, but it
   * has room for one after the other. A screen where the rail is genuinely meaningless on a phone
   * wants a separate route for it, not a pane that is present and off-screen.
   */
  splitAt?: keyof typeof SPLIT_AT;
  /**
   * What separates the panes.
   *
   * - `space` — a gap. Two surfaces on a page, which is what most content splits are.
   * - `line` — flush, with a hairline rule between them. The app shell: a nav rail against a
   *   working surface. Every hand-written one draws this as a `border-r` on the rail, which is
...
/**
 * Two surfaces side by side: a main one and a rail.
 *
 ...
 * `railSize`/`onRailSizeChange` still leaves the drag itself — behaviour — in here, to be
 ...
 * a `StickyHeaderContentFooter` passed as `content` or `rail`, so this shell adds no scroll
 ...
 * of that is `rail={open ? nav : undefined}` — the caller already holds the toggle, and the
 ...
 * different times, and one boolean across both has to either skeleton a nav rail that was never
 ...
 *
 * **Horizontal only.** Stacking below `splitAt` is the vertical arrangement, and a split that is
...
export function SplitPane({
  ...
  const main = (
    <div data-slot="sp-content" className={cn("min-h-0 min-w-0", contentClassName)}>
      {content}
    </div>
  );

  // Rule 5 — an absent slot draws nothing. Not an empty cell, and not a track whose gap is still
  // spent: with no rail there is one column and the main pane has the whole width.
  if (!rail) {
    return (
      <div data-slot="split-pane" className={cn("grid min-h-0 min-w-0 grid-cols-1", className)}>
        {main}
      </div>
    );
  }

  const [railTrack, mainTrack] = TRACKS[railWidth];
  const tracks = railSide === "start" ? [railTrack, mainTrack] : [mainTrack, railTrack];
  ...
  const railCell = (
    <div data-slot="sp-rail" className={cn("min-h-0 min-w-0", railClassName)}>
      {rail}
    </div>
  );
  ...
  return (
    ...
    >
      {railSide === "start" ? railCell : main}
      {divider === "line" ? (
        <div
          data-slot="sp-divider"
          aria-hidden
          className={cn("self-stretch bg-border", DIVIDER_AT[splitAt])}
        />
      ) : null}
      {railSide === "start" ? main : railCell}
    </div>
  );
}
