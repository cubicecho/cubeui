...
export function QueryError({
  ...
  if (compact)
    return (
      <View testID="query-error" className={cn("min-w-0 gap-1 px-2 py-1", className)}>
        <View className="min-w-0 flex-row items-center gap-1.5">
          <TriangleAlert className="h-3.5 w-3.5 shrink-0 text-destructive" aria-hidden />
          <Text className="min-w-0 flex-1 font-medium text-destructive text-xs">
            Could not load {what}
          </Text>
        </View>
        ...
        <View className="flex-row">
          <Button variant="ghost" size="xs" onPress={onRetry}>
